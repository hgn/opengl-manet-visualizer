...
int init_skybox(void)
{
	...
	image[0] = load_image("/usr/share/manet-visualizer/textures/skydome-03/back.bmp", BMP_TYPE);
	if (!image[0])
		die("Can't load texture for skydome");

	image[1] = load_image("/usr/share/manet-visualizer/textures/skydome-03/front.bmp", BMP_TYPE);
	if (!image[1])
		die("Can't load texture for skydome");

	image[2] = load_image("/usr/share/manet-visualizer/textures/skydome-03/down.bmp", BMP_TYPE);
	if (!image[2])
		die("Can't load texture for skydome");

	image[3] = load_image("/usr/share/manet-visualizer/textures/skydome-03/up.bmp", BMP_TYPE);
	if (!image[3])
		die("Can't load texture for skydome");

	image[4] = load_image("/usr/share/manet-visualizer/textures/skydome-03/left.bmp", BMP_TYPE);
	if (!image[4])
		die("Can't load texture for skydome");

	image[5] = load_image("/usr/share/manet-visualizer/textures/skydome-03/right.bmp", BMP_TYPE);
	if (!image[5])
		die("Can't load texture for skydome");
